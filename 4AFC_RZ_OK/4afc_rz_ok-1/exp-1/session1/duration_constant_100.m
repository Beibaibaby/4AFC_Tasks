...
subject_initials    ='FP_constant';
block               = 1;  % 0 means test,run 12 blocks totally
...
%dura_range          =[3 3];%for Olga.
%dura_range          =[0.005 0.2];%for KS.
dura_range          = [0.005 0.15];%RZ,high contrast
%size                = 0.7;    %deg, radius
env_radi            = 11;%1.5;% 2 %11;    %deg, radius.
contrast            = 100;  %3, 5, or 100.
...
orientation            = 0;   %deg
spatial_envelope       = 2;      % 0 = disk, 1 = Gabor patch, 2 = raised cosine envelope, 3 = antCos^3, 4 = cos^3
which_envelope 	       = 2;
...
try
    ...
    elseif spatial_envelope == 3
        R = (sqrt(x.^2 + y.^2) + eps).*circle;R = R/max(max(R));
        cos2D = (cos(R*pi)+1)/2;cos2D = (1 - cos2D).^3; circle = (cos2D.*circle);
    elseif spatial_envelope == 4
        R = (sqrt(x.^2 + y.^2) + eps).*circle;R = R/max(max(R));
        cos2D = (cos(R*pi)+1)/2;cos2D = cos2D.^3; circle = (cos2D.*circle);    
    end
    circleCOS = zeros(bps, bps, 2);
    circleCOS(:,:,2)=(1-circle)*255;
