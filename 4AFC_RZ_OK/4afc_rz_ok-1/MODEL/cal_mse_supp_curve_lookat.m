function [mse, predicts, model_all] = cal_mse_supp_curve_lookat( params, data, fixed_params )
...
for radi_index = 1:length(sizes)
    ...
    for cont_index = 1:length(contrasts)
        ...
        model_all(cont_index, radi_index).mu = mu;
        model_all(cont_index, radi_index).sigma = sigma;
...
if prior_sigma_low < 0 || prior_sigma_high <0 %|| params(7) < 30
 mse = mse + 100000;
end
